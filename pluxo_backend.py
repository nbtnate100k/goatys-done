...
# Telegram multi-message /stock … /done batch upload (context.user_data key).
...
def _stock_field_indices(parts: list[str]) -> dict[str, int]:
...
def build_stock_row_from_line(
    ...
    *,
    ...
    """Build a shop row. Pipe formats:\nPAN|MM/YY|CVV|Name|Street|City|State|ZIP|Country|Phone|…\nPAN|MM|YY|CVV|Name|Street|City|State|ZIP|Country|Phone|…"""
    ...
    if tail:
        b0 = tail[0].upper()
        if b0 in ("VISA", "MASTERCARD", "AMEX", "DISCOVER"):
            row["brand"] = b0
        if len(tail) > 1:
...
def _stock_batch_start(
    ...
    *,
    ...
    context.user_data[STOCK_UPLOAD_SESSION_KEY] = {
        "price": round(float(price), 2),
        "base": base_sel,
...
def _stock_batch_append(context: Any, text: str) -> int:
    """Append a message chunk; return total parsed card line count."""
    sess = _stock_batch_session(context)
    if not sess:
        return 0
...
async def _tg_stock_start_batch(
    ...
    *,
    ...
    _stock_batch_start(context, price=price, base_sel=base_sel, country_code=country_code)
    n = 0
    if first_chunk.strip():
        n = _stock_batch_append(context, first_chunk)
    lbl = html.escape(stock_base_label(base_sel))
    await msg.reply_text(
        f"📥 Upload started — <code>${price:.2f}</code> → <b>{lbl}</b>\n"
        f"Buffered <b>{n}</b> line(s). Paste more card data (long lists may arrive as "
        "several Telegram messages).\n"
        "Send <code>/done</code> when finished, or <code>/cancelstock</code> to abort.",
        parse_mode="HTML",
    )
...
async def tg_cancelstock(update, context) -> None:
    ...
    if not _is_staff(update.effective_user.id):
        await msg.reply_text(TG_AUTH_FAIL)
        return
    had = bool(_stock_batch_session(context))
    _stock_batch_clear(context)
    if had:
...
async def tg_stock_batch_message(update, context) -> None:
    ...
    if not _is_staff(update.effective_user.id):
        return
    if not _stock_batch_session(context):
        return
    n = _stock_batch_append(context, msg.text)
    await msg.reply_text(
        f"Buffered <b>{n}</b> line(s) total. Send <code>/done</code> when finished.",
        parse_mode="HTML",
    )
...
async def tg_stock(update, context) -> None:
    ...
    if re.match(r"^\d{6,19}\|", body):
        sess = _stock_batch_session(context)
        if sess:
            n = _stock_batch_append(context, body)
            await msg.reply_text(
                f"Buffered <b>{n}</b> line(s) total. Send <code>/done</code> when finished.",
                parse_mode="HTML",
            )
    ...
    if m_full:
        ...
        if re.match(r"^\d+(?:\.\d+)?$", body):
            ...
            await _tg_stock_start_batch(
                msg, context, price=price, base_sel=base_sel, country_code=country_code
            )
            return
        ...
        if re.match(r"^\d{6,19}$", g1) and "|" in body:
            ...
            sess = _stock_batch_session(context)
            if sess:
                n = _stock_batch_append(context, blob)
                await msg.reply_text(
                    f"Buffered <b>{n}</b> line(s) total. Send <code>/done</code> when finished.",
                    parse_mode="HTML",
                )
    ...
    if len(cards) > STOCK_BATCH_MAX:
        ...
        await msg.reply_text(
            ...
            "Keep pasting, then <code>/done</code>.",
            parse_mode="HTML",
        )
        return
